  ...
  public function thana($id)
  {
        ...
        if($request->status) {
            switch (strtolower($request->status)) {
                ...
                case 'scheduled':
                    ...
                    break;
                case 'shipped':
                    $data= $data->whereIn('status', ['3','4','5']);
                    break;
                case 'delivered':
    ...
    */
     ...
     public function paymentHistory()
    {
        ...
        $data = MerchantPaymentAccount::selectRaw("*,max(created_at) as max_date,DATE_FORMAT(created_at, '%Y-%m-%d') AS created_date")->where('merchant_id', $merchant_id)->groupBy('txn_id','merchant_id','created_date')->orderBy('max_date','desc')->paginate(10);
        $rdata = [];
        if(!empty($data))
        { 
            ...
            foreach ($data as $key => $value) {
              ...
              $getAllShipment = MerchantPaymentAccount::where('txn_id',$value->txn_id)->where('merchant_id',$value->merchant_id)->whereDate('created_at',$value->created_at)->pluck('shipment_id');
   ...
   /* public function shippingDatabase(Request $request)
     ...
     public function withdrawRequest()
    {
        try{
          ...
          $shipment_ids = MerchantPaymentAccount::where('merchant_id', Auth::user()->id)->pluck('shipment_id');

           $amount = Shipments::join(DB::raw('(select shipment_id, Max(created_at) as date from dri_shipment_assign_status GROUP BY shipment_id) max_user'),'shipment_id','shipments.id')->select('shipments.*','date as created_at','date as max_date')->where('merchant_id',Auth::user()->id)->whereNotIn('id',$shipment_ids)->whereIn('status', ['6','7'])->sum('cod_amount');

            $drivills_commission = Shipments::join(DB::raw('(select shipment_id, Max(created_at) as date from dri_shipment_assign_status GROUP BY shipment_id) max_user'),'shipment_id','shipments.id')->select('shipments.*','date as created_at','date as max_date')->where('merchant_id',Auth::user()->id)->whereNotIn('id',$shipment_ids)->whereIn('status', ['6','7'])->sum('shipment_cost');
         ...
         //$merchant_payment = MerchantPaymentAccount::where('merchant_id', Auth::user()->id)->sum('pay_amount');
          ...
          if(!empty($drivills_commissions))
          {
            ...
            //return $available_payout = $total_cod_collected - $merchant_payment -  $drivills_commission;*/
            $available_payout = $amount -  $drivills_commission;
            ...
            $withdrawRequest->amount = $available_payout; 
     ...
     public function notificationList()
    {
      ...
      if(!empty($data))
      {
        foreach ($data as $key => $value) {
          ...
          $data[$key]->message = str_replace("{{rider_phone_name}}",@$rider->mobile,$value->message);
          if($value->noti_id == 4)
          {
    ...
    {
       $payment = MerchantPaymentAccount::find($id);
       $getAllShipment = MerchantPaymentAccount::where('txn_id',$payment->txn_id)->where('merchant_id',Auth::user()->id)->whereDate('created_at',$payment->created_at)->pluck('shipment_id');
       $shipments = Shipments::select('shipment_no','cod_amount','shipment_cost')->whereIn('id',$getAllShipment)->get();
       return response()->json(['message' =>'list loaded Successfully','data'=>$shipments],200);
    }
    ...
    /*
    |---------------------------------
    | Shipment delete by merchant
    |---------------------------------
    */
/*    public function delete_shipment(Request $request)
    {
        $this->validate($request, [            
            'id' => 'required|exists:shipments,id',
        ]);
        try{
            ...
            if(in_array($shipment->status, ["0","1","2","3"]) || in_array($shipment->status, [0,1,2,3]))
            {
              ...
              $shipment->status_log= ShipmentStatusLog::create([
                  ...
                  'updated_ip'=>$request->ip(),
                  'reason'=>"",
                  'note'=>'Shipment Deleted By Merchant'
              ]);
            }

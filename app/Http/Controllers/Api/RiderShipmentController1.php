...
class RiderShipmentController extends Controller
{
        ...
        try{
            ...
            if($request->status == 6) {
              ...
              if($request->status == 10) {
                  ...
                  $merchant = Merchants::find($shipment->merchant_id);
                  $data = array('title' => 'Customer Not Available','body' => $push_noti->message);
                  sendNotification($merchant->device_token,$data);
    ...
    */
     ...
     public function ridingStatement(Request $request)
    {
        ...
        $max_ids = ShipmentAssignStatus::selectRaw('shipment_id,Max(id) as max_id')->where('rider_id',Auth::user()->id)->groupBy('shipment_id')->pluck('max_id');

        $sql = Shipments::select('shipments.*')->join('shipment_assign_status','shipments.id','=','shipment_assign_status.shipment_id')->whereIn('shipment_assign_status.id',$max_ids);
        ...
        $resp['total_cod_earned']= $riderpayment;
    ...
    public function codStatement()
    {
      ...
      /*  $data = Shipments::select('shipment_no','receiver_name','product_detail','cod_amount','created_at')->whereHas('rider',function($query)
        ...
        $resp['cod_to_deposit']= $cod_to_deposit - $riderdeposite;
     ...
     public function notificationList()
    {
      ...
      if(!empty($data))
      {
        foreach ($data as $key => $value) {
          ...
          $data[$key]->notification = $value->notification.'('.$shipment->shipment_no.')';
        }
      }

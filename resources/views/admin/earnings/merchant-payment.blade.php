                                  ...
                                  <form id="pay_form">
                                    ...
                                    <div class="table-responsive mb-4">
                                        <table class="table table-centered datatable dt-responsive nowrap table-card-list" style="border-collapse: collapse; border-spacing: 0 12px; width: 100%;">
                                            <thead>
                                                <tr class="bg-transparent">
                                                     <th class="nosort">@if(count($remaining_shipments) > 0)<input type="checkbox" id="checkAll" value="">@endif</th>
                                                    <th>Shipment ID</th>
                                                    <th>Customer Name </th>
                                                    <th>COD Amount</th>
                                                    <th>Shipment Charge</th>
                                            ...
                                            <tbody>
                                               @php $cod_amount = $shipment_cost = $total_current = 0; @endphp
                                                ...
                                                <tr style="background-color: @if($current < 0) #FF6251 @else #B2DD8B @endif">
                                                    <td><input type="checkbox" value="{{$s->id}}" class="checkbox_change" data-currentamount="{{$current}}"></td>
                                                    <td>{{$s->shipment_no ?: 'NA'}}</td>
                                                    <td>{{$s->receiver_name}}</td>
                                                    <td>{{$s->cod_amount}}</td>
                                                    <td>{{$s->shipment_cost?: '0'}}</td>
                                                ...
                                                <tr>
                                                    ...
                                                    <td></td>
                                                    <td></td>
                                                    <td>Total COD: {{(int)$cod_amount}}</td>
                                                    <td>Total Charge: {{(int)$shipment_cost}}</td>
                                                    <td>Total Current: {{(int)$total_current}}</td>
...
<script type="text/javascript">
        ...
        var data_rows = table.$(".checkbox_change:checked", {"page": "all"});
                 $("#payAll .modal-header .shipment_id_dv").html('');
                 var selected = [];
                 var shipment_ids = [];
                  $.each(data_rows, function(index,elem) { 
                     var checkbox_value = $(elem).val();    
                     selected.push($(elem).attr('data-currentamount'));       
                     shipment_ids.push($(elem).val());
                     var html = '<input type="hidden" value="'+$(elem).val()+'" name="shipment[]">';       
                     $("#payAll .modal-header .shipment_id_dv").append(html);
                  }); 
